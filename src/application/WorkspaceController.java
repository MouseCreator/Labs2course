...
public class WorkspaceController implements Initializable {
    ...
    private HashMap<Pages, String> destination = new HashMap<>();
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		initDeletionPopup();
		Platform.runLater(()->
		workspacePane.getScene().getWindow().setOnCloseRequest(new EventHandler<WindowEvent>() {
		    @Override
	...
	public Folder refreshLastActive() {
		return lastActiveFolder = new Folder(Folder.ALL_SETS);
	}
	...
	public void loadMenu() {
		try {
			...
			fxmlLoader.setLocation(getClass().getResource("MenuBar.fxml"));
			menuBarPane.getChildren().add(fxmlLoader.load());
			MenuBarController c = (MenuBarController)fxmlLoader.getController();
			c.setParent(this);
		} 
		catch (IOException e) {
			e.printStackTrace();
	...
	public void loadByLastActive(Pages key) {
		loadAttachedToFolder(key, lastActiveFolder);
	}
	...
	private void initFolderWindow() {
		try {
			Parent root;
			FXMLLoader fxmlLoader = new FXMLLoader();
			fxmlLoader.setLocation(getClass().getResource("/application/FolderList.fxml"));
